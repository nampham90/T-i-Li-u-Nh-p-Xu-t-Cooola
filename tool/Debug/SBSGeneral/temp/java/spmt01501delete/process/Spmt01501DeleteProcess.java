package jp.co.brycen.wms.#LowerScreenId#delete.process;
 ...
 * 概要：削除
 */
public class #FirstScreenId#DeleteProcess extends AbstractProcess {
	public #FirstScreenId#DeleteProcess(ILogSender logSender) {
		super(logSender);
	}
	@Override
	protected void beforeProcess(DBAccessor dba, AbstractRequest request, AbstractResponse response,
		...
		#FirstScreenId#DeleteRequest req#FirstScreenId#Delete = (#FirstScreenId#DeleteRequest) request;
		...
		req#FirstScreenId#Exclusion.accessInfo = req#FirstScreenId#Delete.accessInfo;
		req#FirstScreenId#Exclusion.CSTMCD = request.accessInfo.CSTMCD;
		
		#InitVariable#@@req#FirstScreenId#Delete.#LowerScreenId#DeleteCondition;
	...
	public AbstractResponse process(DBAccessor dba, AbstractRequest request, AbstractResponse response,
		...
		//updateDeleteFlag(dba, request);
		updateBuyerHistory(dba, request);
		deleteBuyer(dba, request);

		#FirstScreenId#DeleteResponse res#FirstScreenId#Delete = (#FirstScreenId#DeleteResponse) response;
		return res#FirstScreenId#Delete;
	}
	@Override
	protected AbstractResponse createNewResponse(AbstractRequest request) {
		return new #FirstScreenId#DeleteResponse();
	}
	 ...
	 */
	private String getSequence(DBAccessor dba, #FirstScreenId#DeleteRequest request, String prefix)
	...
	protected void updateDeleteFlag(DBAccessor dba, AbstractRequest request) throws FatalException, DBException, ProcessCheckErrorException {
		DBStatement ps = null;
		try {
			#FirstScreenId#DeleteRequest req#FirstScreenId#Delete = (#FirstScreenId#DeleteRequest) request;
			...
			strSql.append("SET ");
			strSql.append("		DELFLG = 1, ");
			strSql.append("		UPDUSRCD = ? , ");
			strSql.append("		UPDDATETIME = SYSDATETIME(), ");
			strSql.append("		UPDPRG = 'FPMT0150' ");
			strSql.append("WHERE ");
			...
			ps.setString(1, usrcdLogin);
			ps.setString(2, cstmcd);
			ps.setString(3, buyercd);
			ps.executeUpdate();
		} finally {
			if (ps != null)
				ps.close();
	...
	protected void updateBuyerHistory(DBAccessor dba, AbstractRequest request) throws FatalException, DBException, ProcessCheckErrorException {
		...
		try {
			...
			String seqno = this.getSequence(dba, req#FirstScreenId#Delete, "MTHS");

			StringBuilder strSql = new StringBuilder();
			strSql.append("INSERT INTO THS090_BUYER ( ");
			strSql.append(setUpdateFields());
			strSql.append(" ) ");
			strSql.append(" SELECT ");
			strSql.append(setSelectFields(seqno));
			strSql.append("	FROM TMT150_BUYER ");
			strSql.append(" WHERE ");
			strSql.append("	TMT150_BUYER.CSTMCD = ? ");
			strSql.append("	AND TMT150_BUYER.BUYERCD = ?");
			strSql.append(";");

			ps = dba.prepareStatement(strSql);
			ps.setString(1, usrcdLogin);
			ps.setString(2, usrcdLogin);
			ps.setString(3, cstmcd);
			ps.setString(4, buyercd);
			ps.executeUpdate();
		} finally {
			if (ps != null)
				ps.close();
	...
	protected void deleteBuyer(DBAccessor dba, AbstractRequest request) throws FatalException, DBException, ProcessCheckErrorException {
		DBStatement ps = null;
		try {
			#FirstScreenId#DeleteRequest req#FirstScreenId#Delete = (#FirstScreenId#DeleteRequest) request;
			#InitVariable#@@req#FirstScreenId#Delete.#LowerScreenId#DeleteCondition;
